...
IPAddress server2(192, 168, 2, 101);
IPAddress raspberry(192, 168, 2, 101);
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (regen == true) {
      ...
      if (pumpeStart == 0) { // Wenn die Pumpe noch nicht gestartet wurde
        pumpeStart = currentMillis; // Speichere den Startzeitpunkt der Pumpe
        digitalWrite(PUMPE, HIGH); // Starte die Pumpe
      } else {
        if (currentMillis - pumpeStart >= pumpeDauer) { // Überprüfe, ob die Pumpe lange genug gelaufen ist
          digitalWrite(PUMPE, LOW); // Stoppe die Pumpe
          pumpeStart = 0; // Setze den Startzeitpunkt zurück
        }
      }
    }
  }
  ...
  if(currentMillis - previousMillisETH >= 5000){
    ...
    if (client2.connect(server2, 80)) {
      ...
      client2.println("Host: 192.168.2.101"); // die IP des raspis
...
void aktionen(String parameter){
  ...
  char wert= parameter[2];
  int wertascii = int(wert);
  Serial.println(aktion);
  Serial.println(wert);

  if(aktascii == 49 && wertascii == 49){      // ASCII 0=48; 1=49; 2=50; 3=51 ...
    digitalWrite(8, HIGH);
    Serial.println("Hello");
  }else if (aktascii == 49 && wertascii == 48){
    digitalWrite(8, LOW);
  }else if (aktascii == 50 && wertascii == 49){
    digitalWrite(8, HIGH);
  }
}